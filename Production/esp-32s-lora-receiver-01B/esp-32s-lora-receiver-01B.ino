//This is the B version of the Lora receiver, this one uses a socket connection to send messages to the LoRa server.\
    ...
    if(arrayContainsUid==1){ // if an only if true, prepare to parse the sensor value.
      ...
      sensorValue = sValue.toInt(); // check and convert to int if possible.
        ...
        if(sensorValue==1||sensorValue==0){ //if Int value is 1 or 0, prepare to send http message.
          ...
          WiFiClient client;
          if(!client.connect(server, serverPort)){ // If the server connection cannot be established, start the loop() again.
          ...
          client.print(String("GET /?loraUid="+loraUid+"&sensorValue="+String(sensorValue)+" HTTP/1.1")); // Send message uting GET.
          client.stop();                
        }else{
